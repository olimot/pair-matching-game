...
const generateGame = (): GameState => {
  ...
  const combinations: Combination[] = Array.from(Array(49), (_, i) => ({
...
const App = () => {
  ...
  return (
    <div>
      ...
      <h2>
        Score: {Object.keys(solved).length}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Time Left:{' '}
        {timeLeft.toFixed(3)}ms
      </h2>
        ...
        <div className="game-scene">
          ...
          <div className="board">
            {gameState.cells.map(({ index, type, color, isFake }) => (
              <button
                key={index}
                className={selected === index ? 'cell cell--selected' : solved[index] ? 'cell cell--solved' : 'cell'}
                onClick={() => {
                  if (selected === null) {
                    setSelected(index);
                    return;
